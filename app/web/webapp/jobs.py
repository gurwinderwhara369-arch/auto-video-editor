...
from app.cli.render_canvas_reel import run_canvas_pipeline
from app.cli.render_reel import load_or_analyze_metadata, load_or_analyze_moments, run_pipeline
...
def validate_job_config(config: JobConfig) -> None:
    ...
    if config.render_mode not in {"fast", "balanced", "pro_layer"}:
        raise ValueError("Render mode must be fast, balanced, or pro_layer")
...
def run_job(job_id: str, config: JobConfig) -> None:
    ...
    try:
        ...
        _update_status(job_dir, step="Analyzing clips", phase="footage", message="Checking tattoo clip metadata")
        clip_metadata, metadata_path = load_or_analyze_metadata(
            clips_dir=config.clips_dir,
            job_id=job_id,
            requested_path=config.metadata_path,
            generated_path=job_dir / "clip_metadata.json",
        )

        _update_status(job_dir, step="Finding best moments", phase="footage", message="Checking best tattoo moments and crops")
        _, moments_path = load_or_analyze_moments(
            clip_metadata=clip_metadata,
            clips_dir=config.clips_dir,
            requested_path=config.moments_path,
            generated_path=job_dir / "moment_metadata.json",
            max_moments_per_clip=18,
        )

        variants_dir = job_dir / "variants"
        selected_output = job_dir / config.output_name
        variants_dir.mkdir(parents=True, exist_ok=True)
        if config.render_mode == "pro_layer":
            _update_status(job_dir, step="Rendering pro layer reel", phase="render", message="Rendering premium canvas compositor")
            run_canvas_pipeline(
                ...
                temp_dir=job_dir / "_pro_layer_canvas",
            )
            report = _single_output_report("pro_layer", selected_output, variants_dir)
        elif config.render_mode == "fast":
            _update_status(job_dir, step="Rendering fast reel", phase="render", message="Rendering fast FFmpeg output")
            run_pipeline(
                template_path=template_path,
                clips_dir=config.clips_dir,
                song_path=reference_audio,
                ...
                moments_path=moments_path,
                beat_map_path=beat_map_path,
                temp_dir=job_dir / "_fast_segments",
            )
            report = _single_output_report("fast", selected_output, variants_dir)
        else:
...
def _artifacts(job_dir: Path, selected_output: Path, variants_dir: Path) -> dict[str, str]:
    ...
    pro_layer_path = variants_dir / "pro_layer.mp4"
    if pro_layer_path.exists():
        artifacts["pro_layer"] = str(pro_layer_path)
    fast_path = variants_dir / "fast.mp4"
    if fast_path.exists():
        artifacts["fast"] = str(fast_path)
    canvas_timeline = job_dir / "canvas_timeline.json"
    if canvas_timeline.exists():
        artifacts["canvas_timeline"] = str(canvas_timeline)
    return artifacts
...
def _single_output_report(variant: str, selected_output: Path, variants_dir: Path) -> dict[str, Any]:
    variant_output = variants_dir / f"{variant}.mp4"
    shutil.copy2(selected_output, variant_output)
    qa = score_video(selected_output)
    report = {
        "selected_variant": variant,
        "selected_output": str(selected_output),
        "variants": [{"variant": variant, "output": str(variant_output), "qa": qa}],
    }
    (variants_dir / "qa_report.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    return report
...
def _scan_summary(template_path: Path, scan_report_path: Path, beat_map_path: Path) -> dict[str, Any]:
    ...
    visual_summary = scan_report.get("visual_event_summary", {})
    visual_events = scan_report.get("visual_events", [])
    return {
        "name": template.name,
        "duration": round(template.total_duration, 2),
        "segments": len(template.segments),
        ...
        "flashes": len(scan_report.get("flashes", events.get("flash", scan_report.get("flash_events", [])))) if isinstance(events, dict) else 0,
        "visual_events": len(visual_events) if isinstance(visual_events, list) else 0,
        "layers": visual_summary if isinstance(visual_summary, dict) else {},
